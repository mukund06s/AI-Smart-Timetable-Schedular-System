...
class SmartTimetableScheduler:
    ...
    def _evolve_with_progress(self, constraints, generations, ga_progress, ga_status, 
        ...
        # CHANGE: Seed the Graph Coloring initial schedule to ensure 0 missing subjects
        if constraints.get('initial_schedule'):
            seed_ind = self.genetic_algorithm._create_seeded_individual(
                constraints['initial_schedule'], constraints
            )
            population.append(seed_ind)

        # Fill the rest of the population with random individuals
        num_to_create = self.genetic_algorithm.population_size - len(population)
        ...
        with tab2:
            ...
            with col1:
                ...
                if missing:
                    ...
                    if firebase_manager:
                        st.markdown("---")
                        if st.button("📥 Load Previous Datasets", use_container_width=True):
                            upload_mgr = DatasetUploadManager(firebase_manager)
                            
                            # Load info dataset
                            info_data = firebase_manager.get_info_dataset(selected_program, selected_semester)
                            if info_data and 'data' in info_data:
                                st.session_state.info_dataset = info_data['data']
                                st.session_state.subjects = upload_mgr.convert_info_to_subjects(info_data['data'])
                                st.session_state.faculties = upload_mgr.extract_faculty_from_info(info_data['data'])
                                st.session_state.schools_data = upload_mgr.extract_schools_from_info(info_data['data'])
                            
                            # Load room dataset
                            rooms = firebase_manager.get_room_dataset(selected_program)
                            if rooms:
                                st.session_state.room_dataset = rooms
