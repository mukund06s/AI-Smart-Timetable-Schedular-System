        ...
        for idx, row in df.iterrows():
            try:
                ...
                record = {
                    ...
                    'Section': safe_str(get_val(row, 'Section'), 'A'),
                    'Batch': safe_str(get_val(row, 'Batch'), '1'),
        ...
        if selected_school:
            ...
            if selected_program:
                ...
                if selected_semester not in st.session_state.sections[program_key]:
                    st.session_state.sections[program_key][selected_semester] = ['A']
                    ...
                    for _sec in list(_sections_list):
                        ...
                        with _b_col1:
                            if st.button(f"➕ Batch {_next_batch_num}",
                                ...
                                st.session_state.schools_data[program_key] \
                                    .setdefault('batches', {})[selected_semester] = _batches_for_sec
                                st.rerun()
                        with _b_col2:
                            if len(_batches_for_sec) > 1:
                                if st.button(f"➖ Batch {_batches_for_sec[-1]}",
                                    ...
                                    st.session_state.schools_data[program_key] \
                                        .setdefault('batches', {})[selected_semester] = _sb[_sec]
                                    st.rerun()
                ...
                # keep legacy schools_data.batches in sync so the scheduler gets
                # ONE entry per configured section (A, B, C…→ 1, 2, 3…).
                # The scheduler uses these as batch keys: Sem_{year}_Section_{N}.
                # FIXED: previously only section A's batch numbers were stored here,
                # meaning only one schedule was ever generated for all sections.
                _all_sections_list = st.session_state.sections[program_key][selected_semester]
                st.session_state.schools_data[program_key] \
                    .setdefault('batches', {})[selected_semester] = \
                    list(range(1, len(_all_sections_list) + 1))
                    ...
                    if firebase_manager:
                        ...
                        if timetable_data:
                            ...
                            if _loaded_schedule:
                                ...
                                _lsbs_sects = st.session_state.sections.get(_lsbs_prog, {}).get(_lsbs_sem, ['A'])
                                ...
                                for _li, _lsec in enumerate(_lsbs_sects):
                                    _lsbs_sbs[_lsec] = {}
                                    _lbk = (_lsbs_bkeys[_li] if _li < len(_lsbs_bkeys)
                                            else next(iter(_lsbs_flat), None))
                                    _lsec_raw = _lsbs_flat.get(_lbk, {}) if not _lsbs_bkeys else _lsbs_flat.get(_lbk, {})
                                    _lsbs_sbs[_lsec]['_section'] = _copy_load.deepcopy(_lsec_raw)
                                    _lsbs_sbs[_lsec][1] = _copy_load.deepcopy(_lsec_raw)
            ...
            with col1:
                ...
                if info_file:
                    try:
                        ...
                        # Detect potential semester mismatch in the file itself (for warning)
                        if records:
                            file_sem = str(records[0].get('Sem', '')).strip().upper()
                            roman_map = {'I': '1', 'II': '2', 'III': '3', 'IV': '4', 'V': '5', 'VI': '6'}
                            file_sem_num = roman_map.get(file_sem, file_sem)
                            
                            if str(file_sem_num) != str(current_semester):
                                st.warning(f"⚠️ **FILE DATA MISMATCH**: You are uploading for **Semester {current_semester}**, but the file content appears to be for **Semester {file_sem}**. This might cause issues or empty imports.")
                ...
                if st.button("🚀 GENERATE TIMETABLE", type="primary", disabled=not ready):
                    ...
                    if valid_data:
                        with st.spinner("Running AI/ML optimization..."):
                            ...
                            # CHANGE SECTION-BATCH-2 (FIXED v4): Post-process schedule per section.
                            _sel_prog_key = f"{st.session_state.get('selected_school', selected_school)}_{current_program}"
                            _sel_sem = current_semester
                            _gen_sections = st.session_state.sections.get(_sel_prog_key, {}).get(_sel_sem, ['A'])
                            ...
                            for _sec in _gen_sections:
                                _sec_batches = _gen_sb.get(_sec, [1])
                                _sec_num = _gen_sections.index(_sec) + 1
                                _candidate_keys = [
                                    f"Sem_{_sel_sem}_Section_{_sec}",
                                    f"Sem_{_sel_sem}_Section_{_sec_num}",
                                    f"Sem_{_sel_sem}_Sec_{_sec}",
                                    f"Sem{_sel_sem}_Section_{_sec}",
                                    f"Sem{_sel_sem}_Section_{_sec_num}",
                                    f"Section_{_sec}",
                                    f"Sec_{_sec}",
                                    _sec,
                                    str(_sec_num),
                                ]
                                _sec_raw_schedule = None
                                if _flat_by_day:
                                    _sec_raw_schedule = _raw_flat
                                else:
                                    for _ck in _candidate_keys:
                                        if _ck in _raw_flat:
                                            _sec_raw_schedule = _raw_flat[_ck]
                                            break
                                    if _sec_raw_schedule is None and _raw_flat:
                                        _sec_raw_schedule = next(iter(_raw_flat.values()))

                                if not _sec_raw_schedule:
                                    continue

                                _combined_sec_schedule = {}
                                for _day in _raw_days:
                                    _day_slots = _sec_raw_schedule.get(_day, {})
                                    _combined_sec_schedule[_day] = {}
                                    for _slot_key, _ci_raw in _day_slots.items():
                                        if not _ci_raw:
                                            _combined_sec_schedule[_day][_slot_key] = None
                                            continue
                                        
                                        _classes_to_process = _ci_raw if isinstance(_ci_raw, list) else [_ci_raw]
                                        _lab_entries_for_slot = []
                                        _theory_entry_for_slot = None
                                        _fixed_entry = None
                                        ...
                                        for _ci in _classes_to_process:
                                            ...
                                            if _ctype in ('LUNCH', 'BREAK'):
                                                ...
                                                if _b_val.endswith('.0'): _b_val = _b_val[:-2]
                                                _lab_entries_for_slot.append({'batch': _b_val, 'ci': _copy.deepcopy(_ci)})
                                            else:
                                                if _theory_entry_for_slot is None:
                                                    _theory_copy = _copy.deepcopy(_ci)
                                                    _theory_copy.pop('batch', None)
                                                    _theory_entry_for_slot = _theory_copy
                                        
                                        if _fixed_entry:
                                            _combined_sec_schedule[_day][_slot_key] = _fixed_entry
                                        elif _lab_entries_for_slot:
                                            _combined_sec_schedule[_day][_slot_key] = _lab_entries_for_slot
                                        elif _theory_entry_for_slot:
                                            _combined_sec_schedule[_day][_slot_key] = _theory_entry_for_slot
                            ...
                            st.success(f"✅ Created combined timetables for {len(_gen_sections)} section(s). See 'Generated Timetables' tab.")
...
# Continuation from Part 4
        ...
        with tab3:
            ...
            _t3_sbs = (st.session_state.get('section_batch_schedules', {})
                       .get(_t3_prog_key, {})
                       .get(_t3_sem, {}))
